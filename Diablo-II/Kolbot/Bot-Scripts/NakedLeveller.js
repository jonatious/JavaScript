/*
*	@filename	Leveller.js
*	@author		Diabolic
*	@desc		Power leveling script with baal and diablo kill
* 	modded by jony -- leveller needs catacombs lvl 2 and ws keep lvl 2 wps and bRunners[2] - TheRaven in this case should have cata lvl 2 wp as well. Requires 4 bRunners!!!!
...
var getShrine = "Velocity"; // toon that gets the xp shrine
var tpShrine = "TheQueen"; // toon that loohs for and tps the xp shrine
var Levellers = ["Velocity","Raigeki"]; // list of toons that are being leveled
var bRunners = ["TheKnight", "TheQueen","Sharpness","TheEnergy", "TheRaven"]; // Baal runners. First one is the command giver and tp'er
var dRunners = ["DonaldDuck", "TheLight"]; // Diablo runners. First one is the command giver and tp'er
var nRunner = "Snuif";
var diabloPercent = 22; // percent to prep diablo to for quick kill when leecher enters portal
var baalPercent = 20; // percent to prep baal to for quick kill when leecher enters portal
var flag=0;
var flag1=0;
var flag2=0;
var flag3=0;
...
var NakedLeveller = function () {
		...
		function (who, msg) {
			...
			if (who === tpShrine) {
				...
				if (me.name === getShrine) {
					switch (me.area) {
						case 109:
							while(!Pather.usePortal(129, bRunners[2]))
							delay(250);
						...
						case 131:
							...
							Pather.usePortal(129, bRunners[2]);
							Pather.useWaypoint(35, true);
							if(!Pather.usePortal(1, dRunners[1]))
							Pather.useWaypoint(1, true);
	...
	this.leechDiablo = function () {
		...
		Pather.usePortal(103, dRunners[0]);
		Pather.useWaypoint(129, true);
		Pather.usePortal(109, bRunners[2]);
		Town.move("portalspot");
	};
	...
	Attack.hurt(getLocaleString(22435),90); 
	say("shenkR");
	Town.goToTown(5);	
	return true;
	};
	...
	if (!this.getBoss(getLocaleString(2853))) {
		...
		if (me.name !== dRunners[0]) {
			while(!Pather.usePortal(103, null))
			delay(250);
			Town.doChores();
			while (true) {
				delay(1000);
